...
module.exports = app => {
  class MonthServer extends app.Service {
    ...
    * list(pageNum, pageSize) {
      const articles = yield app.mysql.query('select id,name,work_id,price,status,business,is_scene,is_audio,is_model,is_show,is_text,time,scale,channel,timestamp,phone,category_id, platform_id,column_id,video_id,comment from video_bill order by timestamp desc limit ? offset ?;', [ pageSize, (pageNum - 1) * pageSize ]);
      return articles;
    }

    // 获取列表byren
    * listByUser(openid) {
      const articles = yield app.mysql.query('select video_bill.id,video_bill.name,work_id,price,status,business,is_scene,is_audio,is_model,is_show,is_text,time,scale,channel,timestamp,phone,category_id,video_category.name, video_bill.platform_id,video_bill.column_id,video_bill.video_id,comment from video_bill LEFT JOIN video_category on video_bill.category_id=video_category.id LEFT JOIN video_platform on video_bill.platform_id=video_platform.id  LEFT JOIN video_column on video_bill.column_id=video_column.id where openid = ? and status != 3 order by timestamp desc;', [ openid ]);
      return articles;
    }
