...
module.exports = app => {
    class MonthServer extends app.Service {
        ...
        * listByUser(pageNum, pageSize, work_id) {
            const articles = yield app.mysql.query('select video_id, datetime ,video_video.name,video_video.price from video_log LEFT JOIN video_video on video_video.id = video_log.video_id where user_id = ? order by datetime desc limit ? offset ?;', [work_id, pageSize, (pageNum - 1) * pageSize] );
            return articles;
        }
        // 获取某条信息
        * find(id) {
